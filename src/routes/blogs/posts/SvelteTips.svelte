...
<header class="mb-8">
	...
	<div class="flex flex-wrap gap-2 mb-6">
		{#each meta.tags as tag}
			<span class="flex items-center bg-blue-50 px-3 py-1 rounded-full font-medium text-blue-600 text-sm">
...
<article class="max-w-none prose prose-lg">
	...
	<p>SvelteKit 是一个强大的全栈框架，在使用过程中我总结了一些实用的技巧和最佳实践。</p>
	
	<h3>1. 响应式声明的魅力</h3>
	<p>Svelte 的响应式声明让状态管理变得简单直观。使用 <code>$:</code> 标记可以创建自动更新的计算属性：</p>
	...
	<div class="bg-blue-50 mb-6 p-6 border border-blue-200 rounded-lg not-prose">
		...
		<p class="text-gray-700">计数器: <span class="font-bold text-blue-600">{count}</span></p>
		<p class="text-gray-700">两倍值: <span class="font-bold text-green-600">{doubled}</span></p>
		<code class="block bg-gray-100 mt-3 p-3 rounded text-sm">$: doubled = count * 2;</code>
	</div>
...
// 获取查询参数
console.log($page.url.searchParams.get('id'));</code></pre>
	
	<h3>3. 预加载数据</h3>
	<p>使用 <code>+page.ts</code> 或 <code>+layout.ts</code> 文件可以在页面渲染前预加载数据：</p>
	<pre><code>export async function load(&#123; fetch, params &#125;) &#123;
...
&#125;</code></pre>
	...
	<div class="bg-green-50 mb-6 p-6 border border-green-200 rounded-lg not-prose">
		...
		<p class="mt-2 text-gray-600 text-sm">找到 {filteredItems.length} 个结果</p>
	</div>
	
	<h3>5. 组件生命周期</h3>
	<p>理解和合理使用生命周期钩子：</p>
	<pre><code>import &#123; onMount, onDestroy &#125; from 'svelte';
...
$user = &#123; name: '小明', age: 25 &#125;;</code></pre>
	...
	<h3>总结</h3>
	<p>这些技巧可以帮助你更高效地使用 SvelteKit 开发应用。记住，最好的学习方式是实践！</p>
	
	<blockquote>
		<p>💡 <strong>提示</strong>：本文中的交互式演示都是用 Svelte 实现的，展示了框架的强大功能。你可以直接在文章中体验这些特性！</p>
	</blockquote>
</article>
